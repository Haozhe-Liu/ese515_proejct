...
uint8_t variance(int *x, int len){
	uint32_t sum = 0;
	int data_average = average(x, len);
	for (int i=0; i<len; i++)
		sum += pow(x[i]-data_average,2);
	return sum/len;
}
	...
	UART_putstring(String);
   	while (ind < BLOCK_SIZE && control_flag == 1){
		...
		// 1. read from ADC input
   		data[ind] = ADC-300; // set 300 as a bias
		...
		if (ind == BLOCK_SIZE -1){
			...
			uint8_t data_average = average(data, BLOCK_SIZE);
			sprintf(String, "data average: %d \n", data_average);
			UART_putstring(String);
			uint8_t data_variance = variance(data, BLOCK_SIZE);
			sprintf(String, "data covariance: %d \n", data_variance);
			UART_putstring(String);
			for(int j=0;j<5;j++){
			...
			if(data_variance <= 30) data = 3;
			else if(fpeaks[0] < 30 && fpeaks[0] >= 10 && fpeaks[1] > 0 && data_variance > 30) data = 1;
			else data = 2;
